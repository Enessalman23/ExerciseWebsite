...
public class GeminiClientService {
    ...
    public String generateContent(String prompt, int maxRetries, boolean expectJson) {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                lastException = e;
                retryCount++;
                log.error("Gemini API attempt {} failed: {}", retryCount, e.getMessage());
    ...
    public String generateContentWithImage(String prompt, String mimeType, String base64Data, int maxRetries, boolean expectJson) {
        String geminiEndpoint = "https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=" + geminiApiKey;
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                lastException = e;
                retryCount++;
                log.error("Gemini API image attempt {} failed: {}", retryCount, e.getMessage());
                if (retryCount < maxRetries) {
                    try { 
                        Thread.sleep(2000); 
                    } catch (InterruptedException ie) {
        ...
        throw new RuntimeException("Failed to get response from Gemini API with image after " + maxRetries + " attempts. Last error: " + (lastException != null ? lastException.getMessage() : "Unknown"));
    }
    ...
    private String extractJson(String text) {
        ...
        if (startObj == -1 || endObj == -1 || endObj < startObj) {
            log.error("CRITICAL: No valid JSON found in AI response: {}", text);
