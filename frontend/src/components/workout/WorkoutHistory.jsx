...
import { PREDEFINED_WORKOUTS } from '../../constants/predefinedWorkouts';
...
const WorkoutHistory = ({ 
  ...
  const [activeTab, setActiveTab] = useState('custom'); // custom, predefined

  const itemsToRender = activeTab === 'custom' ? historyItems : PREDEFINED_WORKOUTS;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
             ...
             <div>
               <h2 style={{ fontSize: '2.2rem', margin: 0, fontWeight: 900, letterSpacing: '-1px' }}>Program Kütüphanesi</h2>
               <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', fontWeight: 600 }}>
                 {activeTab === 'custom' ? 'Sizin için hazırlanan yapay zeka programları' : 'Uzmanlar tarafından hazırlanan standart programlar'}
               </p>
             </div>
           </div>
           {activeTab === 'custom' && (
        ...
        <div style={{ display: 'flex', gap: '15px', borderBottom: '1px solid var(--border-color)', paddingBottom: '15px' }}>
          <button 
            ...
            style={{
              ...
              borderRadius: '14px',
              border: '1px solid',
              borderColor: activeTab === 'custom' ? 'var(--primary)' : 'var(--glass-border)',
              background: activeTab === 'custom' ? 'var(--primary)' : 'var(--glass-bg)',
              color: activeTab === 'custom' ? '#fff' : 'var(--text-muted)',
              fontWeight: 800,
              fontSize: '0.9rem',
              cursor: 'pointer',
              transition: 'all 0.3s ease',
              boxShadow: activeTab === 'custom' ? '0 5px 15px var(--primary-glow)' : 'none'
            }}
          >
          ...
          <button 
            ...
            style={{
              ...
              borderRadius: '14px',
              border: '1px solid',
              borderColor: activeTab === 'predefined' ? '#0ea5e9' : 'var(--glass-border)',
              background: activeTab === 'predefined' ? '#0ea5e9' : 'var(--glass-bg)',
              color: activeTab === 'predefined' ? '#fff' : 'var(--text-muted)',
              fontWeight: 800,
              fontSize: '0.9rem',
              cursor: 'pointer',
              transition: 'all 0.3s ease',
              boxShadow: activeTab === 'predefined' ? '0 5px 15px rgba(14, 165, 233, 0.2)' : 'none'
            }}
          >
            🔥 Hazır Antrenmanlar
          </button>
        ...
        {activeTab === 'custom' && loading && itemsToRender.length === 0 ? (
